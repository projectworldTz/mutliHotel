...
<div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
    <div class="mb-4 flex items-center gap-2">
        <a href="{{ route('account.bookings') }}" class="btn-ghost btn-sm">← My Bookings</a>
    </div>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="card p-6">
                <div class="flex flex-wrap items-start justify-between gap-3 mb-5">
                    <div>
                        <h1 class="text-xl font-bold text-slate-900 dark:text-white">Booking #{{ $booking->booking_number }}</h1>
                        <p class="text-sm text-slate-500 mt-0.5">Made on {{ $booking->created_at->format('d M Y') }}</p>
                    </div>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div class="rounded-xl bg-slate-50 dark:bg-slate-700/50 p-4">
                        ...
                        <p class="text-xs text-slate-500">from {{ $booking->hotel->check_in_time ?? '14:00' }}</p>
                    </div>
                    <div class="rounded-xl bg-slate-50 dark:bg-slate-700/50 p-4">
                        ...
                        <p class="text-xs text-slate-500">by {{ $booking->hotel->check_out_time ?? '11:00' }}</p>
                    </div>
                </div>

                <div class="mt-4 grid grid-cols-3 gap-3 text-sm">
                    <div><span class="text-slate-500">Nights</span><p class="font-semibold mt-0.5">{{ $booking->nights }}</p></div>
                    <div><span class="text-slate-500">Adults</span><p class="font-semibold mt-0.5">{{ $booking->guests_adults }}</p></div>
                    <div><span class="text-slate-500">Children</span><p class="font-semibold mt-0.5">{{ $booking->guests_children }}</p></div>
                </div>
                ...
                <div class="mt-4 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 p-3 text-sm">
                    <p class="font-semibold text-amber-700 dark:text-amber-400 mb-1">Special Requests</p>
            ...
            {{-- Hotel info --}}
            <div class="card p-6">
                <h3 class="font-bold text-slate-900 dark:text-white mb-3">Hotel</h3>
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <a href="{{ route('hotels.show', $booking->hotel) }}" class="text-sm text-navy dark:text-navy-light underline">View hotel →</a>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="p-5 border-b border-slate-100 dark:border-slate-700">
                    <h3 class="font-bold text-slate-900 dark:text-white">Booked Rooms</h3>
                </div>
                <div class="divide-y divide-slate-100 dark:divide-slate-700">
                    @foreach($booking->rooms as $room)
                    <div class="p-5 flex items-center justify-between gap-4">
                        <div>
                            <p class="font-semibold text-slate-900 dark:text-white">{{ $room->roomType->name ?? 'Room' }}</p>
        ...
        <div class="space-y-4">
            ...
            <div class="card p-5 text-sm space-y-2">
                <h3 class="font-bold text-slate-900 dark:text-white mb-3">Price Summary</h3>
                <div class="flex justify-between"><span class="text-slate-500">Subtotal</span><span>TZS {{ number_format($booking->sub_total, 0) }}</span></div>
                @if($booking->discount_total > 0)
                <div class="flex justify-between text-emerald-600"><span>Discount</span><span>−TZS {{ number_format($booking->discount_total, 0) }}</span></div>
                @endif
                <div class="flex justify-between"><span class="text-slate-500">Tax ({{ $booking->tax_rate }}%)</span><span>TZS {{ number_format($booking->tax_total, 0) }}</span></div>
            ...
            <div class="card p-5">
                ...
                <form method="POST" action="{{ route('booking.cancel', $booking) }}"
                      x-data x-on:submit.prevent="if(confirm('Are you sure you want to cancel this booking?')) $el.submit()">
                    @csrf
            ...
            <a href="{{ route('booking.invoice', $booking) }}"
               class="btn-outline w-full block text-center">Download Invoice</a>
            @endif
        </div>
    </div>
</div>
