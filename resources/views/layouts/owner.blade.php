...
<div class="flex min-h-screen">
    ...
    <aside class="fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-navy transition-transform duration-300 ease-in-out
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
            ...
            @if($myHotel->hasFeature('maintenance_requests'))
            <a href="{{ route('owner.maintenance.index', $myHotel) }}"
               class="{{ str_starts_with($route ?? '', 'owner.maintenance') ? 'nav-link-active' : 'nav-link' }}">
                <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @if($myHotel->hasFeature('staff_scheduling'))
            <a href="{{ route('owner.shifts.index', $myHotel) }}"
               class="{{ str_starts_with($route ?? '', 'owner.shifts') ? 'nav-link-active' : 'nav-link' }}">
                <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @if($myHotel->hasFeature('group_booking'))
            <a href="{{ route('owner.group-bookings.index', $myHotel) }}"
               class="{{ str_starts_with($route ?? '', 'owner.group-bookings') ? 'nav-link-active' : 'nav-link' }}">
                <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m5-3.13a4 4 0 100-8 4 4 0 000 8zm6 3c1.657 0 3 1.79 3 4v2h-3m-9-6a4 4 0 00-4 4v2h9v-2a4 4 0 00-4-4z"/>
                </svg>
                {{ __('Group Bookings') }}
