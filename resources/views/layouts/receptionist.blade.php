...
<div class="flex min-h-screen">
    ...
    <aside class="fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-navy transition-transform duration-300 ease-in-out
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
            ...
            @if(isset($assignedHotel) && $assignedHotel->hasFeature('maintenance_requests'))
            <a href="{{ route('receptionist.maintenance.index') }}"
               class="{{ str_starts_with($route ?? '', 'receptionist.maintenance') ? 'nav-link-active' : 'nav-link' }}">
                <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @if(isset($assignedHotel) && $assignedHotel->hasFeature('guest_messaging'))
            <a href="{{ route('receptionist.messages.index') }}"
               class="{{ str_starts_with($route ?? '', 'receptionist.messages') ? 'nav-link-active' : 'nav-link' }}">
                <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
                </svg>
                @php
                    $navMsgUnread = \App\Models\GuestMessage::forHotel($assignedHotel->id)->unread()->fromGuest()->count();
                @endphp
                <span class="flex-1">{{ __('Messages') }}</span>
                @if($navMsgUnread > 0)
                <span class="ml-auto inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-rose-500 px-1.5 text-[10px] font-bold text-white">
                    {{ $navMsgUnread > 99 ? '99+' : $navMsgUnread }}
                </span>
                @endif
            </a>
            @endif

            @if(isset($assignedHotel) && $assignedHotel->hasFeature('digital_checkin'))
            <a href="{{ route('receptionist.checkins.index') }}"
               class="{{ str_starts_with($route ?? '', 'receptionist.checkins') ? 'nav-link-active' : 'nav-link' }}">
                <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
                <span>{{ __('Check-ins') }}</span>
            </a>
            @endif

            @if(isset($assignedHotel) && $assignedHotel->hasFeature('staff_scheduling'))
            <a href="{{ route('receptionist.shifts.index') }}"
               class="{{ str_starts_with($route ?? '', 'receptionist.shifts') ? 'nav-link-active' : 'nav-link' }}">
                <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                </svg>
